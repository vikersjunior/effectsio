...
export function findPaletteValueFromHex(hex: string): PaletteControlValue {
  const normalized = hex.toLowerCase();
  for (const family of TAILWIND_COLOR_PALETTE) {
    for (const [shade, shadeHex] of Object.entries(family.shades)) {
      if (shadeHex.toLowerCase() === normalized) {
        return { family: family.name as PaletteColorFamily, shade: shade as PaletteShadeStep };
      }
    }
  }
  return { family: "Orange", shade: "500" };
}
