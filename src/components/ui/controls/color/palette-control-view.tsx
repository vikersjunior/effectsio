...
export function PaletteControlView(props: PaletteControlViewProps) {
  const paletteGrid = (
    <div
      className="grid content-start gap-x-2 gap-y-2"
      style={{
      ...
      {STYLE_GUIDE_PRIMARY_FAMILY_OPTIONS.map((palette) => {
        ...
        return (
          <button
            ...
            className={cn(
              "relative size-[24px] place-self-center rounded-full border border-[color:color-mix(in_oklab,var(--foreground)_10%,transparent)] transition-[box-shadow,transform,opacity] duration-150 ease-out hover:scale-[1.04] active:scale-[0.98]",
              "focus-visible:ring-2 focus-visible:ring-[color:color-mix(in_oklab,var(--ring)_40%,transparent)] focus-visible:outline-hidden",
              isSelected &&
                "after:pointer-events-none after:absolute after:-inset-[4px] after:rounded-full after:border-2 after:border-[color:var(--foreground)] after:content-['']",
  ...
  const shadeRail = (
    <div className="flex items-stretch">
      ...
      >
        ...
        >
          <div className="absolute inset-[-3px] rounded-[7px] border-[2.5px] border-[color:var(--foreground)] [box-shadow:0_0_4px_rgba(0,0,0,0.3),inset_0_0_4px_rgba(0,0,0,0.3)]" />
        </div>
        ...
        {PALETTE_SHADE_STEPS.map((shade, index) => {
          ...
          return (
            <button
              ...
              disabled={props.disabled}
              className={cn(
                "relative inline-flex min-h-0 w-[18px] flex-1 rounded-none border border-transparent transition-[opacity,transform] duration-150 ease-out hover:scale-[1.02]",
                "focus-visible:ring-2 focus-visible:ring-[color:color-mix(in_oklab,var(--ring)_40%,transparent)] focus-visible:outline-hidden",
  ...
  if (props.variant === "panel") {
    return (
      <div
        ...
        className={cn("flex w-full justify-center py-[10px]", props.className)}
        role="group"
      >
        <div className="inline-flex w-fit items-stretch">
          ...
          <div
            aria-hidden="true"
            className="mx-3.5 w-px shrink-0 bg-[color:color-mix(in_oklab,var(--border)_8%,transparent)]"
          />
          <div
