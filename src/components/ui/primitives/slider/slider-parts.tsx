...
import type { SliderThumbDoubleClickHandler } from "./slider-types";

export type SliderOrientation = "horizontal" | "vertical";

export type SliderControlContentProps = {
  count: number;
  disabled?: boolean;
      ...
      {Array.from({ length: count }, (_, index) => (
        ...
        >
          <span
            ...
            className={cn(
              "pointer-events-none absolute inset-0 block rounded-[2px] bg-[color:var(--slider-active-color)] transition-[scale,background-color] duration-200 ease-out",
              disabled ? null : "group-hover/slider-thumb:scale-[1.4] data-[active-dragging=true]:scale-[1.4]",
            )}
          />
        </SliderPrimitive.Thumb>
      ))}
...
export function SliderFill({ show }: { show: boolean }): React.JSX.Element | null {
  if (!show) {
    return null;
  }
  ...
  return (
    <SliderPrimitive.Indicator
      ...
      className={cn(
        "bg-[color:var(--slider-active-color)] transition-[width,height,inset-inline-start,bottom] duration-200 ease-out select-none data-[dragging]:transition-none data-horizontal:h-full data-vertical:w-full",
      )}
    />
  );
}

export function SliderControlContent({
  count,
  disabled,
  getAriaLabel,
  isDiscrete,
