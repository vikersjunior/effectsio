...
8. **Rule 8 — Non-Destructive Editing & Source of Truth State**: Original asset bitmaps remain intact and immutable. `activeFrameId + activeLayerId` is the single authoritative source of truth for the active editing target in the Unified Composition Model (`activeImageId` is strictly a derived compatibility property for image-source layers). Never introduce duplicate active selection state in toolbars, panels, or other UI surfaces.
